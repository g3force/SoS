...
public class CourseManager {
	...
	private Course[]					courses;																		// not really
																																// extensible, is it?
	private Course						curCourse;
	private String						savepath	= System.getProperty("user.dir") + "//courses.xml";	// are you crazzy? :o I
																																// do not want a
																																// "courses.xml"
																																// directly in my home
																																// folder ;)
						...
						for (int k = 0; k < students[j].length; k++) {
							...
							if (students[j][k].getClass().getName().contains("EmptyPlace")) {
								...
								for (int m = 0; m < currentStudent.getActualState().size(); m++) {
									...
									// TODO parameter namensliste muss hier zur verfügung stehen und code entsprechend geändert
									// werden, da in den vektoren keine parameter namen mehr gespeichert werden sondern nur
									// zahlen
				...
				if (reader.hasName()) {
					...
					try {
						...
						if (tagname.contentEquals("courses")) {
							...
							// TODO muss ebenfalls angepasst werden, da mir mitgeteilt wurde, dass sich die anzahl der
							// parameter zur programmlaufzeit nicht ändert und einem student daher kein neuer parameter
							// hinzugefügt werden kann
							Parameter p = new Parameter(reader.getAttributeValue(0), Integer.parseInt(reader
									.getAttributeValue(1)));
