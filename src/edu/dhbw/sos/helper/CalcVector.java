...
public class CalcVector implements Cloneable {
	...
	public CalcVector multiplyWithInteger(int constant) {
		...
		for (int i = 0; i < this.params.size(); i++) {
			result.vector.get(i).setValue(this.vector.get(i).getValue()*constant);
		}
		return result;
	}
	/**
	...
	public CalcVector multiplyWithDouble(double constant) {
		...
		for (int i = 0; i < this.vector.size(); i++) {
			result.vector.get(i).setValue((int)(this.vector.get(i).getValue()*constant));
		}
		return result;
	}
	public CalcVector multiplyWithMatrix(Matrix m) {
		...
		for (int i = 0; i < result.size(); i++) {
			...
			for (int j = 0; j < result.size(); j++) {
				value += this.vector.get(i).getValue() * m.getElementAt(i, j).getValue();
			}
	...
	public CalcVector clone() {
		...
		for (int i = 0; i < this.size(); i++) {
			result.vector.get(i).setValue(this.vector.get(i).getValue());
		}
		return result;
	}
	//debug method
