...
// import edu.dhbw.sos.gui.plan.data.MovableBlock;
// import edu.dhbw.sos.gui.plan.data.MovableBlocks;
...
// * @author NicolaiO
// */
// public void initMoveableBlocks() {
// start = 0;
...
// // tbs.clear();
// // for (MoveableBlock mb : movableBlocks) {
...
//
// // 3 possiblilites: leftmost block (leftBlock is null), middle block, and rightmostblock (rightBlock is
// // null)
// // not the first block
// if (leftBlock != null && leftBlock.getWidth() + moveX < MIN_BLOCK_WIDTH) {
// // Checks if the width of right Block is lower or equal then MIN_BLOCK_WIDTH
...
//
// leftBlock.setLocation(moveBlock.getLocation().getX() + moveBlock.getWidth() - MIN_BLOCK_WIDTH,
// leftBlock.getY());
...
// index = newIndex;
//
// moveBlock.setLocation(moveBlock.getLocation().x - MIN_BLOCK_WIDTH, moveBlock.getLocation().y);
...
// rightBlock.printMbTb(index + 1, "R");
//
// // Swap the moving Block and its right neighbor
// newIndex = movableBlocks.swap(index, index + 1);
//
// leftBlock = movableBlocks.get(newIndex - 1);
...
// savedWidthLeft = (int) leftBlock.getWidth();
//
// leftBlock.setLocation(movableBlocks.get(newIndex).getX() - leftBlock.getWidth() + MIN_BLOCK_WIDTH,
// leftBlock.getY());
//
// if (newIndex + 1 < movableBlocks.size()) {
// savedWidthRight = (int) rightBlock.getWidth();
...
// moveBlock.setLocation(moveBlock.getLocation().x + MIN_BLOCK_WIDTH, moveBlock.getLocation().y);
// } else {
// // 1. check if movement is possible
// if (leftBlock != null) {
// if (moveX < 0 && leftBlock.getWidth() + moveX < MIN_BLOCK_WIDTH) {
// // left block would be to small
// return;
// }
// } else if (moveX < 0 && moveBlock.getWidth() + moveX < MIN_BLOCK_WIDTH) {
// // moveBlock gets too small
// return;
// }
// if (rightBlock != null) {
// if (moveX > 0 && rightBlock.getWidth() + moveX < MIN_BLOCK_WIDTH) {
// // right block would be to small
// return;
// }
// } else if (moveX > 0 && moveBlock.getWidth() - moveX < MIN_BLOCK_WIDTH) {
// // moveBlock gets too small
...
// }
//
// // 2. now we are save to move any block
//
...
// }
//
// // 3. Horizontal Movement of the actual block
